...
public class DPScope extends Observable
{
	...
	static
	{
		...
		mapTimeDivs.put(DIV_1_S, 1.0);
		mapTimeDivs.put(DIV_500_MS, 1.0);
		mapTimeDivs.put(DIV_200_MS, 1.0);
		mapTimeDivs.put(DIV_100_MS, 1.0);
		mapTimeDivs.put(DIV_50_MS, 1.0);
		mapTimeDivs.put(DIV_20_MS, 1.0);
		mapTimeDivs.put(DIV_10_MS, 1.0);
		mapTimeDivs.put(DIV_5_MS, 1.0);
		mapTimeDivs.put(DIV_2_MS, 1.0);
		mapTimeDivs.put(DIV_1_MS, 1.0);
		mapTimeDivs.put(DIV_500_US, 1.0);
		mapTimeDivs.put(DIV_200_US, 1.0);
		mapTimeDivs.put(DIV_100_US, 1.0);
		mapTimeDivs.put(DIV_50_US, 1.0);
		mapTimeDivs.put(DIV_20_US, 1.0);
		mapTimeDivs.put(DIV_10_US, 1.0);
	...
	static
	{
		...
		mapSamplingRates.put(DIV_1_S, DIV_10_SA);
		mapSamplingRates.put(DIV_500_MS, DIV_20_SA);
		mapSamplingRates.put(DIV_200_MS, DIV_50_SA);
		mapSamplingRates.put(DIV_100_MS, DIV_100_SA);
		mapSamplingRates.put(DIV_50_MS, DIV_200_SA);
		mapSamplingRates.put(DIV_20_MS, DIV_500_SA);
		mapSamplingRates.put(DIV_10_MS, DIV_1_KSA);
		mapSamplingRates.put(DIV_5_MS, DIV_2_KSA);
		mapSamplingRates.put(DIV_2_MS, DIV_5_KSA);
		mapSamplingRates.put(DIV_1_MS, DIV_10_KSA);
		mapSamplingRates.put(DIV_500_US, DIV_20_KSA);
		mapSamplingRates.put(DIV_200_US, DIV_50_KSA);
		mapSamplingRates.put(DIV_100_US, DIV_100_KSA);
		mapSamplingRates.put(DIV_50_US, DIV_200_KSA);
		mapSamplingRates.put(DIV_20_US, DIV_500_KSA);
		mapSamplingRates.put(DIV_10_US, DIV_1_MSA);
	...
	/*
	 * ARM parameters (watch out, VB array is 1-based, MikroC array is 0-based) 1:
	 * command 2: first channel to acquire 3: second channel to acquire 4:
	 * acquisition parameters (sample clock divider, acquisition time) 5: timer 0
	 * pre-load high 6: timer 0 preload low 7: timer 0 prescaler bypass (0 = use
	...
	/*
	 * parameter for "software gain": (shift,subtract) = (2,0), (1,128), or (0,192)
	 * for gain 1, 2, 4
	 */
	...
	public volatile float[] scopeBuffer = new float[448];
	...
	{
		CMD_IDLE, CMD_PING, CMD_REVISION, CMD_ARM, CMD_DONE, CMD_ABORT, CMD_READBACK, CMD_READADC, CMD_STATUS_LED, CMD_WRITE_MEM,
		CMD_READ_MEM, CMD_WRITE_EEPROM, CMD_READ_EEPROM, CMD_READ_LA, CMD_ARM_LA, CMD_INIT, CMD_SERIAL_INIT, CMD_SERIAL_TX,
		CMD_CHECK_USB_SUPPLY, EVT_DEVICE_REMOVED;
	}
				...
				hidDev.setInputReportListener(new InputReportListener()
				{
					...
					public void onInputReport(HidDevice source, byte Id, byte[] rxBuf, int len)
					{
						...
						case CMD_CHECK_USB_SUPPLY:
							usbSupplyVoltage = (float) 4.096 * 1023 / ((int) (rxBuf[0] & 0xFF) * 256 + (rxBuf[1] & 0xFF));
		...
		actionQueue.add(new BootAction()
		{
			...
			public boolean go() throws Exception
			{
				...
				*/
				...
				{
					/*
					 * external trigger uses fixed threshold of approx. 1.5V (compatible with TTL,
					 * 5V CMOS and 3.3V CMOS)
					 */
